...
public class Factory { //静态工厂
    ...
    public static  <T> T createBean(String property) { //将配置文件中要得到的属性名传进来
        ...
        try {
            ...
            //通过流的方式去加载配置文件
            properties.load(new FileInputStream("src/main/resources/object.properties"));

            //通过类的加载器getClassLoader 加载发布到Tomcat下文件资源
            //properties.load(Factory.class.getClassLoader().getResourceAsStream("object.properties"));
